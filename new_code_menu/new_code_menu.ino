...
void loop() {
  ...
  // EEPROMWritelong(maxReadingAddress, 447);
  // EEPROMWritelong(minReadingAddress, 356);
...
void calibration(int interval, int calibProcess){
  ...
  while(calibProcess == 1){
    ...
    if (previousValue == currentValue){  
      ...
      }else if (tempvalue != 0 && tempvalue != currentValue){
        counter =0;
        tempvalue = currentValue;
      ...
      // Serial.print("Counter triggered at :");
    ...
    previousValue = currentValue;
     ...
     if (counter == 4 && currentValue < start &&  upwardSwitch == 0){
      digitalWrite(pinOut, LOW);
      Serial.println("Solenoid stopped");//stop the solenoidd
      //calibProcess = 0;
      counter =0;
      upwardSwitch = 1;
    }else if (upwardSwitch == 1 && currentValue >= end){
      Serial.println("line 362");
